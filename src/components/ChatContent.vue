...
const conversitionList = computed(() => {
  return store.conversitionList.filter(
    (x: Conversition) =>
      (x.SendId == store.sender.Id && x.ReciverId == store.reciver.Id) ||
      (x.ReciverId == store.sender.Id && x.SendId == store.reciver.Id)
  );
});
...
function readySend() {
  ...
  store.initEditor();
  let len =
    store.sessionList.filter((x: any) => x.Id == store.readyReciver.Id)
      ?.length ?? 0;
  if (len === 0) {
    store.sessionList.push(store.readyReciver);
    let query = {
      SendId: store.sender.Id,
      Revicer: store.readyReciver,
    };
      ...
      <div class="content-win">
        <n-scrollbar style="max-height: 100%" ref="contentScrollbar">
          <template v-for="(item, index) in conversitionList" :key="index">
            ...
            <div v-else-if="item.SendId == store.reciver.Id">
